...
class Admin extends Admin_Controller
{
    ...
    { 		
        ...
        $studentMonthlyLeave = $this->apply_leave_model->getStudentMonthlyLeave($start_date, $end_date);
        $studentApproveMonthlyLeave = $this->apply_leave_model->getStudentApproveMonthlyLeave($start_date, $end_date);
        $data['getStudentMonthlyLeave'] = $getStudentMonthlyLeave = is_countable($studentMonthlyLeave) ? count($studentMonthlyLeave) : 0;
        $data['getStudentApproveMonthlyLeave'] = $getStudentApproveMonthlyLeave = is_countable($studentApproveMonthlyLeave) ? count($studentApproveMonthlyLeave) : 0;
        ...
        $staffMonthlyLeave = $this->apply_leave_model->getStaffMonthlyLeave($start_date, $end_date);
        $data['getStaffMonthlyLeave'] = $getStaffMonthlyLeave = is_countable($staffMonthlyLeave) ? count($staffMonthlyLeave) : 0;
		
		
        $staffApproveMonthlyLeave = $this->apply_leave_model->getStaffApproveMonthlyLeave($start_date, $end_date);
        $data['getStaffApproveMonthlyLeave'] = $getStaffApproveMonthlyLeave = is_countable($staffApproveMonthlyLeave) ? count($staffApproveMonthlyLeave) : 0;
        ...
        if (is_countable($tot_head_students)) {
    ...
    public function dtstudentlist()
    {
        ...
        $resultlist      = $this->student_model->searchFullTextAdmin($search_text, $carray);
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                ...
                $viewbtn    = '';
                $collectbtn = "";
                $viewbtn    = "<a href='" . base_url() . "student/view/" . $student->id . "'   class='btn btn-primary btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder'></i></a>";
                ...
                $row[] = "<div class='d-flex gap-0-5 text-right float-right'>".$viewbtn . '' . $editbtn . '' . $collectbtn."</div>";
                $dt_data[] = $row;
            }
