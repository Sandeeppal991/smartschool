        ...
        (function() {																																		   
            ...
            function applyLayoutAndIcons() {
                ...
                if (navigationIcon) {
                    ...
                    // Ensure we have fas class
                    if (!navigationIcon.classList.contains('fas')) {
                        navigationIcon.classList.add('fas');
                    }
                    
                    navigationIcon.classList.toggle('fa-bars', isCollapsed);
                    navigationIcon.classList.toggle('fa-th-large', !isCollapsed);
                }

                if (layoutIcon) {
                    // Ensure we have fas class
                    if (!layoutIcon.classList.contains('fas')) {
                        layoutIcon.classList.add('fas');
                    }
                ...
                if (shadowIcon) {
                    // Ensure we have fas class
                    if (!shadowIcon.classList.contains('fas')) {
                        shadowIcon.classList.add('fas');
                    }
                    
                    shadowIcon.classList.toggle('fa-layer-group', themeShadow === 'shadow-applied');
                    shadowIcon.classList.toggle('fa-square', themeShadow !== 'shadow-applied');
                }
...
if ($this->module_lib->hasActive('chat')) {
    ...
    <li class="cal15 d-sm-none">
        ...
        if (is_array($msg_count) || $msg_count instanceof Countable) {
            echo count($msg_count);
        } elseif (is_numeric($msg_count)) {
            echo (int)$msg_count;
        } else {
            echo 0;
        }
...
$image = $result["image"];
$role  = (is_object($role_obj) && !empty($role_obj->name)) ? $role_obj->name : '';
$id    = $result["id"];
if (!empty($image)) {
                                            ...
                                            <li>
                                                <div class="sstopuser">
                                                    ...
                                                    <div class="sspass">
                                                        ...
                                                        if(is_object($getfrontcmssetting) && !empty($getfrontcmssetting->is_active_front_cms)){  ?>
